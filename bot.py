...
def main():
    ...
    # ── Миграция: если pickle пуст (первый запуск на Railway),
    #    сидируем registered_users чтобы рассылка работала сразу.
    _SEED_USERS = {807869, 514493805}
    if not app.bot_data.get("registered_users"):
        app.bot_data["registered_users"] = _SEED_USERS
        logger.info(f"Seeded registered_users: {_SEED_USERS}")
